//
//  dataFileIFReduced.cpp
//  AMPLIFI
//
//  Created by Ningyu Liu on 6/7/22.
//  Adapted from from DataFileIFReduced.cpp in the Chombo package
...
DataFileIFReduced::DataFileIFReduced(const DataFileIFReduced::DataType& a_dataType,
                       const Real&                 a_value,
                       const bool&                 a_inside,
...
DataFileIFReduced::DataFileIFReduced(const char* const           a_filename,
...
DataFileIFReduced::DataFileIFReduced(const DataFileIFReduced& a_inputIF)
{
...
DataFileIFReduced::DataFileIFReduced(const RefCountedPtr<FArrayBox>               a_ascii_data,
...
DataFileIFReduced::~DataFileIFReduced()
{
}

void DataFileIFReduced::GetHeader(IntVect&  a_num,
                           RealVect& a_spacing,
...
void DataFileIFReduced::GetParams(Real& a_value,
                           bool& a_inside,
...
void DataFileIFReduced::SetParams(const Real& a_value,
                           const bool& a_inside,
                           const bool& a_useCubicInterp)
{
...
void DataFileIFReduced::SetNoDataValue(const Real& a_value)
{
...
Real DataFileIFReduced::value(const IndexTM<Real,GLOBALDIM>& a_point) const
{
  return value(IndexTM<int,GLOBALDIM>::Zero,a_point);
}

Real DataFileIFReduced::value(const IndexTM<int,GLOBALDIM> & a_partialDerivative,
...
void DataFileIFReduced::OpenFile(ifstream&         a_file,
                          const char* const a_filename)
{
  ...
  if (!a_file.good())
  {
    MayDay::Abort("DataFileIFReduced::OpenFile - Unable to open data file");
  }
}

void DataFileIFReduced::CloseFile(ifstream& a_file)
{
  a_file.close();
}

void DataFileIFReduced::ReadMinHeader(IntVect& a_num,
                               istream& a_file)
{
  char curLine[1024];
  int length = 1024;
  ...
  {
    MayDay::Abort("DataFileIFReduced::ReadMinHeader - Unable to read number of grid points in each direction from data file");
  }
  else
  {
    if (numFound > SpaceDim)
...
void DataFileIFReduced::ReadFullHeader(IntVect& a_num,
                                RealVect& a_spacing,
                                RealVect& a_origin,
                                istream&  a_file)
{
  ...
  if (numFound < SpaceDim)
  {
    MayDay::Abort("DataFileIFReduced::ReadFullHeader - Unable to read number of grid points in each direction from data file");
  }
  else
  {
    if (numFound > SpaceDim)
  ...
  if (numFound < SpaceDim)
  {
    MayDay::Abort("DataFileIFReduced::ReadFullHeader - Unable to read number of grid spacing from data file");
  }
  else
  {
    if (numFound > SpaceDim)
  ...
  if (numFound < SpaceDim)
  {
    MayDay::Abort("DataFileIFReduced::ReadFullHeader - Unable to read number of grid origin from data file");
  }
  else
  {
    if (numFound > SpaceDim)
...
void DataFileIFReduced::ReadData(Real&                       a_maxValue,
                          istream&                    a_file,
                          const DataFileIFReduced::DataType& a_dataType,
                          const IntVect&              a_num)
{
  ...
  if (a_dataType == DataFileIFReduced::ASCII)
  {
    ...
    while (state[SpaceDim-1] < a_num[SpaceDim-1])
    {
      ...
      if (a_dataType == DataFileIFReduced::Binary)
      {
  ...
  else if (a_dataType == DataFileIFReduced::Binary)
  {
...
void DataFileIFReduced::MakeCorners(void)
{
